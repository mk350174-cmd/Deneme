...
function unifiedPipelineMode() {
  return String(process.env.UNIFIED_PIPELINE_MODE || '').toLowerCase() === 'true';
}
...
class DailyAutomation {
  ...
  async runDailyContentGeneration() {
    // Unified pipeline: content is researched (A-P1), strategised (B) and
    // produced (A-P2/P3) upstream; this agent only reviews, publishes and learns.
    if (unifiedPipelineMode()) {
      this.logger.info('UNIFIED_PIPELINE_MODE: skipping self-generated content (A/B branches own research, strategy and production)');
      return { skipped: true, reason: 'unified_pipeline_mode' };
    }
    try {
  ...
  async weeklyStrategyReview() {
    if (unifiedPipelineMode()) {
      this.logger.info('UNIFIED_PIPELINE_MODE: strategy is owned by B-Branch; export the learning feed instead (integration/export-learning-feed.js)');
      return { skipped: true, reason: 'unified_pipeline_mode' };
    }
    try {
