...
import type { B11CanonicalState, B11StatePersistence } from "./types.js";

export class InMemoryB11Persistence implements B11StatePersistence {
  private versions: Map<string, B11CanonicalState> = new Map();

  async save(state: B11CanonicalState): Promise<void> {
  ...
  async load(version: string): Promise<B11CanonicalState | null> {
  ...
  async latest(): Promise<B11CanonicalState | null> {
    const versions = Array.from(this.versions.keys());
