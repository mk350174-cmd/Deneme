// B07 Persistence Layer

import type { B07CanonicalState, B07StatePersistence } from "./types.js";

export class InMemoryB07Persistence implements B07StatePersistence {
  private versions: Map<string, B07CanonicalState> = new Map();

  async save(state: B07CanonicalState): Promise<void> {
  ...
  async load(version: string): Promise<B07CanonicalState | null> {
  ...
  async latest(): Promise<B07CanonicalState | null> {
    const versions = Array.from(this.versions.keys());
