...
/**
 * Latest-state check (not historical presence): true only if the MOST
 * RECENT record for this gate has the given state_after. Preserved from
 * the pre-repair P3 gates.ts — used by callers that need a boolean rather
 * than a thrown error, and by the revocation-semantics tests (an approval
 * followed by a rejection must read as NOT approved).
 */
export function hasGateState(gates: GateApprovalRecord[], gateId: string, requiredStateAfter: string): boolean {
  const latest = [...gates].reverse().find((g) => g.gate_id === gateId);
  return latest?.state_after === requiredStateAfter;
}
